...
namespace SHS.Sage.Linq.Translation
{
    ...
    public class QueryBinder : DbExpressionVisitor
    {
        ...
        protected Dictionary<Expression, GroupByInfo> GroupByMap {  get { return groupByMap; } }
        ...
        public IdentifiableAlias GetNextAlias()
        {
            return new IdentifiableAlias();
        }

        protected virtual ProjectedFields ProjectFields(Expression expression, IdentifiableAlias newAlias, params IdentifiableAlias[] existingAliases)
        {
            return FieldProjector.ProjectFields(this.language, expression, null, newAlias, existingAliases);
        }
        ...
        protected override Expression VisitMethodCall(MethodCallExpression m)
        {
            ...
            else if (m.Method.DeclaringType.Implements(typeof(IRepository)))
            {
                IIdentifiableSet upd = this.batchUpd != null
                    ? this.batchUpd
        ...
        private IIdentifiableSet GetSetFromExpression(Expression expression)
        {
            if (expression is ConstantExpression)
            {
                return ((ConstantExpression)expression).Value as IIdentifiableSet;
            }
            else if (expression is UnaryExpression)
            {
                return ((ConstantExpression)((UnaryExpression)expression).Operand).Value as IIdentifiableSet;
            }
            else throw new NotSupportedException("Cannot retrieve set from the given expression.");
        }

        private Expression BindConcat(MethodCallExpression m)
        {
            Expression obj = this.Visit(m.Object);
            IEnumerable<Expression> args = this.VisitExpressionList(m.Arguments);
            var sourceType = args.ElementAt(0).Type;
            var targetType = m.Method.GetParameters()[0].ParameterType;
            if (targetType.Implements(typeof(IQueryable))
                && sourceType.Implements(typeof(IEnumerable)))
            {
                var projection0 = args.ElementAt(0) as ProjectionExpression;
                args = new Expression[]
                {
                    Visit(new ProjectionExpression(projection0.Select, projection0.Projector, Aggregator.GetAggregator(targetType, sourceType))),
                    args.ElementAt(1)
                };
            }
        ...
        protected virtual ProjectionExpression VisitSequence(Expression source)
        {
            // sure to call base.Visit in order to skip my override
            return this.ConvertToSequence(base.Visit(source));
        }

        protected virtual ProjectionExpression ConvertToSequence(Expression expr)
        {
        ...
        protected virtual Expression BindWhere(Type resultType, Expression source, LambdaExpression predicate)
        {
        ...
        private Expression BindReverse(Expression source)
        {
            ...
            ProjectedFields pc = this.ProjectFields(projection.Projector, alias, projection.Select.Alias);
            return new ProjectionExpression(
                new SelectExpression(alias, pc.Fields, projection.Select, null).SetReverse(true),
                pc.Projector
                );
        }
        ...
        private Expression BindSelect(Type resultType, Expression source, LambdaExpression selector)
        {
            ...
            var alias = this.GetNextAlias();
            ProjectedFields pc = this.ProjectFields(expression, alias, projection.Select.Alias);
            return new ProjectionExpression(
                new SelectExpression(alias, pc.Fields, projection.Select, null),
                pc.Projector
                );
        }
        ...
        protected virtual Expression BindIntersect(Expression outerSource, Expression innerSource, bool negate)
        {
            ...
            Expression exists = new ExistsExpression(
                new SelectExpression(new IdentifiableAlias(), null, innerProjection.Select, innerProjection.Projector.Equal(outerProjection.Projector))
                );
            if (negate)
                exists = Expression.Not(exists);
            var alias = this.GetNextAlias();
            ProjectedFields pc = this.ProjectFields(outerProjection.Projector, alias, outerProjection.Select.Alias);
            return new ProjectionExpression(
                new SelectExpression(alias, pc.Fields, outerProjection.Select, exists),
        ...
        protected virtual Expression BindGroupJoin(MethodInfo groupJoinMethod, Expression outerSource, Expression innerSource, LambdaExpression outerKey, LambdaExpression innerKey, LambdaExpression resultSelector)
        {
            ...
            var alias = this.GetNextAlias();
            ProjectedFields pc = this.ProjectFields(resultExpr, alias, outerProjection.Select.Alias);
            return new ProjectionExpression(
                new SelectExpression(alias, pc.Fields, outerProjection.Select, null),
                pc.Projector
                );
        }
        ...
        protected virtual Expression BindOrderBy(Type resultType, Expression source, LambdaExpression orderSelector, OrderType orderType)
        {
            ...
            ProjectedFields pc = this.ProjectFields(projection.Projector, alias, projection.Select.Alias);
            return new ProjectionExpression(
                new SelectExpression(alias, pc.Fields, projection.Select, null, orderings.AsReadOnly(), null),
                pc.Projector
                );
        }
        ...
        protected Expression CurrentGroupElement {  get { return currentGroupElement; } }

        protected class GroupByInfo
        {
            public IdentifiableAlias Alias { get; private set; }
            public Expression Element { get; private set; }
            public GroupByInfo(IdentifiableAlias alias, Expression element)
            {
                this.Alias = alias;
        ...
        private Expression BindDistinct(Expression source)
        {
            ...
            ProjectedFields pc = this.ProjectFields(projection.Projector, alias, projection.Select.Alias);
            return new ProjectionExpression(
                new SelectExpression(alias, pc.Fields, projection.Select, null, null, null, true, null, null, false),
                pc.Projector
                );
        }
        ...
        private Expression BindTake(Expression source, Expression take)
        {
            ...
            ProjectedFields pc = this.ProjectFields(projection.Projector, alias, projection.Select.Alias);
            return new ProjectionExpression(
                new SelectExpression(alias, pc.Fields, projection.Select, null, null, null, false, null, take, false),
                pc.Projector
                );
        }
        ...
        private Expression BindSkip(Expression source, Expression skip)
        {
            ...
            ProjectedFields pc = this.ProjectFields(projection.Projector, alias, projection.Select.Alias);
            return new ProjectionExpression(
                new SelectExpression(alias, pc.Fields, projection.Select, null, null, null, false, skip, null, false),
                pc.Projector
                );
        }
        ...
        private Expression GetSingletonSequence(Expression expr, string aggregator)
        {
            ...
            var alias = this.GetNextAlias();
            var colType = this.language.TypeSystem.GetStorageType(expr.Type);
            SelectExpression select = new SelectExpression(alias, new[] { new FieldDeclaration("value", expr, colType) }, null, null);
            return new ProjectionExpression(select, new FieldExpression(expr.Type, colType, alias, "value"), gator);
        }
        ...
        protected override Expression VisitConstant(ConstantExpression c)
        {
            if (this.IsQuery(c))
            {
                ...
                IIdentifiableSet t = q as IIdentifiableSet;
                if (t != null)
                {
                    IHaveMappingEntity ihme = t as IHaveMappingEntity;
                    var entity = ihme != null ? ihme.Entity : this.mapper.Mapping.GetEntity(t.ElementType, this.repoType);//, t.TableId);
                ...
                else if (q.Expression.NodeType == ExpressionType.Constant)
                {
                    ...
                    var entity = this.mapper.Mapping.GetEntity(q.ElementType, this.repoType);
                    return this.VisitSequence(this.mapper.GetQueryExpression(entity));
                }
                else
                {
                    var pev = PartialEvaluator.Eval(q.Expression, this.mapper.Mapping.CanEvaluateLocally);
        ...
        protected override Expression VisitMemberAccess(MemberExpression m)
        {
            if (m.Expression != null
                ...
                && this.IsQuery(m))
            {
                return this.VisitSequence(this.mapper.GetQueryExpression(this.mapper.Mapping.GetEntity(m.Member, this.repoType)));
            }
        ...
        public static Expression BindMember(Expression source, MemberInfo member)
        {
            ...
            if (source.Type.Equals(typeof(IThing))
                || source.Type.Equals(typeof(IAssociation))
                || source.Type.Equals(typeof(IIdentifiable)))
            {
